...
ILOLAZYCONSTRAINTCALLBACK5(SubtourEliminationCallback, IloArray<IloArray<IloBoolVarArray>>, y, int, Stations, int, nBuses, int, N, int, C) {
	...
	for (i = 0; i < nBuses; i++) {
		...
		while (true) {
			...
			else if (length < n && length > 0 || sta.size()==0) break;
			else if (length == 0) {
				//cout << " choose 2 ----- \n";
				if (k < n - 1)k++;
				else {
...
void main() {
	...
	const int N = 5; // amounf of mandatory stations
	inst << "Number of mandatory bus stops: " << N << endl;
	const int M = 3; // anount of stations in cluster
	...
	const int C = 20; // amount of clients in opt horizon
	...
	for (int i = 0; i < C; i++) {
		...
		for (int j = 0; j < Stations; j++) {
			if(traveltimep[i][j]>d || (j>N && traveltimep[i][j]>minp)) inst << "/\t";
			else inst << int(traveltimep[i][j]) << "\t";
		}
		inst << endl;
	}
	...
	for (const auto& p : P) {
		...
		for (const auto& j : J) {
			if (traveltimep[p][j] < mindist)mindist = traveltimep[p][j];
		}
	...
	*/
	...
	std::cout << "Computational time: " << elapsed_time << " seconds " << endl << endl;
